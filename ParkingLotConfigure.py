...
try:
    with open('./Bounding Boxes/rectangles.pkl', 'rb') as file:
        rectangles = pickle.load(file)
        d_rectangles = pickle.load(file)
except:
    print(f'rectangles.pkl does not exist')

while True:
    lot_img = cv2.imread('./Data/baseParkingImg.jpg')
    ...
    if cv2.waitKey(25) & 0xFF == ord('q'):
        with open('./Bounding Boxes/rectangles.pkl', 'wb') as file:
